/**
 * @file Network_DC.cpp
 * @brief Dreamcast networking backend — Phase-1 "not present" stub.
 *
 * Maps the engine's NET_* surface to no-ops so the build links and boots
 * without a network stack. A real port can drive KOS's lwIP sockets over the
 * Broadband Adapter here; SocketHandle is int32 (NetworkTypes_Platform.h).
 *
